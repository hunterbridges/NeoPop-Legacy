...
{
	joy_count = 0;
	IDirectInput_EnumDevices(di, DI8DEVTYPE_GAMEPAD, CountJoysticksCallback,
...
char* system_input_stickname(int i)
{
	...
	joy_count = 0;
	IDirectInput_EnumDevices(di, DI8DEVTYPE_GAMEPAD, NameJoysticksCallback,
...
int system_input_init(void)
{
	...
	if FAILED(DirectInput8Create(g_hInstance, DIRECTINPUT_VERSION, (const IID*)&IID_IDirectInput8A, &di, NULL))
		return 0;
	...
	// Find the joystick
	joy_count = 0;
	IDirectInput_EnumDevices(di, DI8DEVTYPE_GAMEPAD, EnumJoysticksCallback,
		NULL, DIEDFL_ATTACHEDONLY);
